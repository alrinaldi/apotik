...
$cek_penjualan[$j] = $this->db->query("SELECT * FROM penjualan where tgl = '$tgl1' and  id_obat = '$id_obat_fsn[$i]' ");
$cekj[$j] = $cek_penjualan[$j]->num_rows(); 
        foreach ($cek_penjualan[$j]->result_array() as $x) {
          ...
          $cek_pembelian[$j] = $this->db->query("SELECT * FROM pembelian where tgl = '$tgl1' and  id_obat = '$id_obat_fsn[$i]' ");
        ...
        foreach ($cek_pembelian[$j]->result_array() as $x) {
          ...
          if($cekj[$j]==0 or $cekb[$j]==0){
            //echo "kosong </br>";
            $hd[$i]= $stokobat[$i]+$hd[$i];
                //echo $q[$j];
          }else{
            $hd[$i] = $stokobat[$i]+$q[$j]-$qj[$j];
            $rq[$i] = $qp[$j];
            //echo $id_obatj[$j];
